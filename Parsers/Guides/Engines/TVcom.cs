namespace RoliSoft.TVShowTracker.Parsers.Guides.Engines
{
    ...
    public class TVcom : Guide
    {
        ...
        public override TVShow GetData(string id)
        {
            ...
            var show    = new TVShow
                {
                    Title       = HtmlEntity.DeEntitize(summary.DocumentNode.GetValue("//meta[@property='og:title']", "content")),
                    Genre       = Regex.Replace(summary.DocumentNode.GetValue("//h4[text()='Genre']/following-sibling::p[1]") ?? string.Empty, @"\s+", string.Empty).Replace(",", ", "),
                    Description = HtmlEntity.DeEntitize((summary.DocumentNode.GetValue("//p[starts-with(@class, 'show_description')]") ?? string.Empty).Replace("&hellip; More", string.Empty)),
                    Cover       = summary.DocumentNode.GetValue("//meta[@property='og:image']", "content"),
                    Airing      = !Regex.IsMatch(summary.DocumentNode.GetValue("//h4[text()='Status']/following-sibling::p[1]") ?? string.Empty, "(Canceled|Ended)"),
                    Runtime     = 30,
                    Episodes    = new List<TVShow.Episode>()
                };

            var airinfo = summary.DocumentNode.GetValue("//span[@class='tagline']");
            if (airinfo != null)
            {
            ...
            {
                var meta   = episode.GetValue(".//div[@class='meta']");
                ...
                show.Episodes.Add(new TVShow.Episode
                    {
                        ...
                        Airdate = DateTime.TryParseExact(aired.Groups[1].Value, "M/d/yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out dt)
                                  ? dt
                                  : Utils.UnixEpoch,
                        Title   = HtmlEntity.DeEntitize(episode.GetValue(".//h3").Trim()),
                        Summary = HtmlEntity.DeEntitize(episode.GetValue(".//p[@class='synopsis']").Trim()),
                        Picture = episode.GetValue(".//div[@class='THUMBNAIL']/a/img", "src")
                    });
            }
        ...
        public override string GetID(string name)
        {
            ...
            var url  = html.DocumentNode.SelectSingleNode("//li//h2/a").GetAttributeValue("href", string.Empty);
