// added NetFx45 support - RoliSoft
...
begin
	...
	if (version = NetFx10) then begin
        ...
        begin
          RegQueryDWordValue(HKLM, netfx11plus_reg + 'v4\Full' + lcid, 'Install', regVersion);
          RegQueryDWordValue(HKLM, netfx11plus_reg + 'v4\Full' + lcid, 'Release', releaseVer);
        end
		end;
		if (version = NetFx45) then begin
      Result := (regVersion <> 0) and (releaseVer >= 378389);
		end else begin
      Result := (regVersion <> 0);
		end;
	end;
